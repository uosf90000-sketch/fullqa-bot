...
async function sweep(profile,cookie,paths){for(const path of paths){try{const r=await fetch(BASE+path,{headers:cookie?{cookie}:{},redirect:'follow',signal:AbortSignal.timeout(30000)});const text=await r.text();const final=new URL(r.url).pathname;const fatal=/(Application error|Internal Server Error|500 Internal|This page could not be found)/i.test(text);const loginRedirect=final==='/login'&&!['/login','/register'].includes(path);ck(profile,path,r.status===200&&!fatal&&!loginRedirect,`status=${r.status} final=${final}${fatal?' fatal-marker':''}${loginRedirect?' unexpected-login':''}`)}catch(e){ck(profile,path,false,String(e))}}}
